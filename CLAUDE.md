...
- **`/Users/muhash/flash-moe/`** (this repo) — Self-contained package. Core in `flash_moe/lazy_experts/`. Depends on stock `mlx-lm>=0.30.0` (no fork needed).
- **`/Users/muhash/mlx-lm/`** (local fork, `lazy-experts` branch) — Development mirror of `flash_moe/lazy_experts/`. Keep in sync when making core changes.
